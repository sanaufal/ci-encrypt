    ...
    function index() {
        ...
        if ($action) {
            ...
            if ($action == 'load') {
                ...
                $params = array();
                $datas = $this->Client_model->get_all_suppliers($params, $search, $limit, $start, $order, $dir);
                $ret->data = $datas;
                $totaldata = $this->Client_model->get_count_suppliers($params, $search);
                $ret->recordsTotal = $totaldata;
                $ret->recordsFiltered = $totaldata;
                $ret->stat = 1;
            }
            if ($action == 'save') {
                $autonumber = $this->autonumber('CL');
                $params = array(
                ...
                $data = $this->Client_model->save($params);
                $ret = array('stat' => 1, 'data' => $data);
            }
            if ($action == 'edit') {
                $id = $this->input->post('id');
                $data = $this->Client_model->edit($id);
                $ret = array('stat' => 1, 'data' => $data);
            }
            if ($action == 'update') {
                $id = $this->input->post('id');
                ...
                $data = $this->Client_model->update($id, $params);
                $ret = array('stat' => 1, 'data' => $data);
            }
            if ($action == 'delete') {
                $id = $this->input->post('id');
                $params = array('contact_stat' => $this->input->post('stat'));
                $stat = $this->Client_model->delete($id, $params);
                if ($stat == true) {
                    $ret = array('stat' => 1);
                } else {
                    $ret = array('stat' => 0);
            ...
            $data['_submenu'] = 'client';
            $data['_view'] = 'client/index';
            $data['_script'] = 'client/script';
