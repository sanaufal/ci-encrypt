<script>
    $(document).ready(function() {
        var url = "<?php echo site_url('supplier/index'); ?>"
        var dataLength = 10;
        var defaultTxtOnInit = '';
        var city = $('#city').select2({
            allowClear: true,
            ajax: {
                url: "<?= base_url('supplier/getCity'); ?>",
                dataType: 'json',
                type: 'post',
                delay: 250,
                global: false,
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (parseInt(result.stat) === 1) {
                        ...
                        $('#data_id').val(data['contact_id']).attr('disabled', false);
                        $('#code').val(data['contact_code']).attr('disabled', false);
        ...
        $(document).on("click", ".view-data", function(e) {
            ...
            $("#Create").hide();
            $("#Cancel").text('Close');
            $('.modal-title').text('Detail Supplier');
            var data = {
                action: 'edit',
                id: $(this).data('id')
            };
